...
def connect_to_redis():
    ...
    port = 7001
    db = 0
    r = redis.Redis(host=host, port=port, db=db)  # 连接
    return r
...
# 传入地区获取缩写
def get_short_naem(r, name):
    short_name = r.get(name)
    return short_name

# 查询票价，存入redis
def check_ticket_price(r, train_num, train_no, from_station_no, to_station_no, seat_types, train_date):
    year = train_date[0:4]
...
# 检查班次
def check_ticket_nums(r, from_station, to_station, time=None):
    thread = []
    if time == None:
        time = str(time.strftime('%Y-%m-%d', time.localtime(time.time())))
    from_station_shortname = get_short_naem(r, from_station)
    to_station_shortname = get_short_naem(r, to_station)
    ...
    for i in ticket_info_list:
        ...
        if is_reserve == '预订':
            ...
            train_start_chinses_name = get_short_naem(r, train_start_local).decode('utf-8')  # 列车起点中文名
            train_end_chinses_name = get_short_naem(r, train_end_local).decode('utf-8')  # 列车终点中文名
            ...
            name = threading.Thread(target=check_ticket_price, args=(r, train_num, train_no, from_station_no, to_station_no, seat_types, train_date))
            thread.append(name)
    return thread

def write_to_html(r, time, from_station, to_station):
    ...
    with open('climb_ticket_price.html', 'wb') as fp:
        ...
        for train_num in train_num_list:
            ...
            tempmap['num']=train_num
            tempmap['local']=train_other_list[-2]+'/'+train_other_list[-1]
            tempmap['time']=train_other_list[3] + '/' + train_other_list[4]
            tempmap['needtime']=train_other_list[5]
            tempmap['se']=train_other_list[-4] + '|' + train_other_list[-3]
            tempmap['tdz']=train_other_list[6] + '|' + train_price_list[0]
            tempmap['ydz']=train_other_list[7] + '|' + train_price_list[1]
            tempmap['edz']=train_other_list[8] + '|' + train_price_list[2]
            tempmap['gjrw']=train_other_list[9] + '|' + train_price_list[3]
            tempmap['rw']= train_other_list[10] + '|' + train_price_list[4]
            tempmap['dw']=train_other_list[11] + '|' + train_price_list[5]
            tempmap['yw']=train_other_list[12] + '|' + train_price_list[6]
            tempmap['rz']=train_other_list[13] + '|' + train_price_list[7]
            tempmap['yz']=train_other_list[14] + '|' + train_price_list[8]
            tempmap['wz']= train_other_list[15] + '|' +train_price_list[9]
...
def get_price(from_station, to_station, time):
    r = connect_to_redis()
    save_to_redis(r)
    threadlist = check_ticket_nums(r, from_station, to_station, time)
    if threadlist == None:
        connect_to_redis('stop')
        exit(-1)
    for t in threadlist:
        t.setDaemon(True)
        t.start()
    t.join()
    data = write_to_html(r, time, from_station, to_station)
    control_redis('stop')
    print('all over.')
    return data

def control_redis(cmd):
    if cmd == 'start':
        try:
            call(["/Users/ryan/app/redis/7001/src/redis-server", "/Users/ryan/app/redis/7001/redis.conf"])
        except Exception as e:
            print("start redis failed: %s" % e)
            exit(-1)
    elif cmd == 'stop':
        try:
            call(["sh", "/Users/ryan/app/redis/7001/src/killredis.sh"])
        except Exception as e:
            print("close redis failed: %s" % e)
            exit(-1)
